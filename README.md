...
本工具提供了一个简单直观的图形用户界面，用于测试到指定服务器IP或域名的TCP端口是否开放，并测量连接延迟。它模仿了Windows原生`ping`工具的输出格式，并提供了实时的统计结果。

## ✨ 功能特性

- **图形用户界面**：所有操作均在GUI窗口中完成，无需记忆命令行参数。
- **高DPI兼容**：界面在高分屏（如2K, 4K）上也能清晰显示，不会模糊。
- **现代化外观**：UI控件采用Windows现代视觉样式，并提供了自定义图标。
- **批量Ping**：可自定义Ping的次数，支持无限次Ping（输入-1或0）。
- **实时日志**：模仿Windows `ping`命令的格式，实时显示每个连接请求的结果。
- **统计报告**：在手动停止或任务完成后，自动统计并显示最大/最小/平均延迟，以及成功/失败次数和丢包率。
- **右键菜单**：日志框支持右键“复制”和“全选”，方便提取数据。
